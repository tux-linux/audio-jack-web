...
iterations_total_header="${iterations_total}-${#SEND_SOURCE}"
echo "POCSAG2400:1:\"==IT=${iterations_total_header}\"" | ./pagerenc | ffmpeg -hide_banner -loglevel error -f s16le -ar 22050 -ac 1 -i - -ar 44100 "${BASEPATH}/iterations.wav" && play "${BASEPATH}/iterations.wav" || error_exit "Failed sending total iterations to client" 2
